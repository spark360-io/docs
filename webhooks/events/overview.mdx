...
<Note>
  Phase 2 (May 2026) changed the lead payload shape by moving `widget_version` inside
  `source_tracking`. This is a breaking shape change for old consumers, but no
  production subscribers existed when shipped, so migration cost was zero.
</Note>
...
## Source tracking — system fallback values
